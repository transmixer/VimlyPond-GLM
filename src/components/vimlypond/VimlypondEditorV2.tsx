...
import { useVimlypondStoreV2 } from '@/lib/vimlypond/store-v2';
import { useVexFlowRendererV2 } from '@/lib/vimlypond/renderer-v2';
import { t } from '@/lib/vimlypond/i18n';
import { midiToPitch } from '@/lib/vimlypond/music-v2';
import type { NoteRect } from '@/lib/vimlypond/types-v2';
...
export default function VimlypondEditorV2() {
  ...
  const { renderScore, vfReady } = useVexFlowRendererV2(
    containerRef,
    score,
    setNoteRects
  );
  ...
  useEffect(() => {
    ...
    const allStaffs = score.groups.flatMap(g => g.staves);
    const staff = allStaffs[cursorPos.staffIndex];
    if (!staff) return;
    ...
    if (cursorPos.mode === 'normal') {
      const selectedIndex = Math.min(cursorPos.elementIndex, voice.elements.length - 1);

      if (selectedIndex >= 0 && voice.elements.length > 0) {
      ...
      } else if (cursorPos.elementIndex <= voice.elements.length) {
        rect = noteRects.find(r =>
          r.staffIndex === cursorPos.staffIndex &&
          r.measureIndex === cursorPos.measureIndex &&
  ...
  const getInputPreview = useCallback(() => {
    ...
    if (cursorPos.elementIndex > 0 && voice.elements.length > 0) {
      ...
      if (lastEl.type === 'note') {
        const note = lastEl as any;
        const pitch = midiToPitch(note.pitches[0].midiPitch);
        let str = pitch.name.toUpperCase();
        if (note.pitches[0].alter === 1) str += '#';
        else if (note.pitches[0].alter === -1) str += 'b';
        str += pitch.oct + ' dur:' + note.duration;
        if (note.dots > 0) str += '.'.repeat(note.dots);
        return str;
      } else {
  ...
  return (
    <div className="h-screen flex flex-col bg-[#f8f9fa]" style={{ colorScheme: 'light' }}>
      <header className="flex items-center justify-between px-6 py-4 border-b border-[#dee2e6] bg-white">
        <div className="flex items-center gap-4">
          <h1 className="text-xl font-bold tracking-tight">
            ...
            <span className="text-xs text-[#868e96] ml-2">v2</span>
          </h1>
          <div
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="status-pill">
            ...
            <span className="value">
              {allStaffs[cursorPos.staffIndex]?.clef === 'treble' ? 'G' : 'F'}
            </span>
          </div>
        </div>
      ...
      <style jsx global>{`
        ...
        @media (prefers-color-motion: reduce) {
          *, *::before, *::after {
